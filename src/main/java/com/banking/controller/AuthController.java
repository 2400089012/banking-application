...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody AuthRequest request) {
        ...
        if (request.getCaptchaToken() != null && request.getCaptchaAnswer() != null) {
            try {
                Object answerObj = jwtUtil.extractClaim(request.getCaptchaToken(), (Claims claims) -> claims.get("answer"));
                Integer answer = null;
                ...
                if (answer == null || !request.getCaptchaAnswer().equals(answer)) {
                    return ResponseEntity.badRequest().body(Map.of("error", "Incorrect CAPTCHA answer. Expected: " + answer + ", but got: " + request.getCaptchaAnswer()));
                }
            } catch (Exception e) {
                e.printStackTrace();
                return ResponseEntity.badRequest().body(Map.of("error", "Invalid CAPTCHA token: " + e.getMessage()));
            }
        }
